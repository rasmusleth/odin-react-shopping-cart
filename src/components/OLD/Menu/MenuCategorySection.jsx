...
const MenuItem = ({ item, onItemClick }) => {
  ...
  return (
    <>
      <a
        className={`${cartItem ? styles.menuItemWrapperAdded : styles.menuItemWrapper}`}
        onClick={(e) => onItemClick(item, e)}>
        <div className={styles.menuItemContent}>
          <h3 className={`${styles.menuItemTitle} textStyle2Lines`}>{item.title}</h3>
          <p className={`${styles.menuItemDescription} textStyle2Lines`}>{item.description}</p>
          <div className="spacerXXSmall"></div>
          <p className={styles.menuItemPrice}>{formatPrice(item.price)}</p>
        </div>
        <img className={styles.menuItemImage} src={item.image} alt={item.title} />
    ...
    <>
      <section
        ...
        <div className={styles.menuCategoryTitleContainer}>
          <a href={`#${category.slug.slice(1)}`} className={styles.menuCategoryTitleWrapper}>
            <img
        ...
        <div className={styles.menuItemList}>
          {category.items.map((item) => (
            <MenuItem onItemClick={onItemClick} key={`categoryItem-${item.id}`} item={item} />
          ))}
        </div>
      </section>
